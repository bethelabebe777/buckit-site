# Privacy Policy

Published April 1, 2026

1. Information We Collect
   We collect information you provide when using BuckIt, including account details, goals, sessions, and notes.

2. How We Use Information
   We use your information to provide and improve the app experience, including tracking progress and storing sessions.

3. Data Storage
   Your data is stored securely and used only for app functionality.

4. Third-Party Services
   We may use Apple services (such as Sign in with Apple) to authenticate users.

5. Your Rights
   You can request deletion of your data at any time by contacting support.

6. Contact
   For questions or concerns, contact:
   [buckitsupport@gmail.com](mailto:buckitsupport@gmail.com)
